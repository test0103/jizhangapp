<template>
    <div class="Remember">
        ...
        <div v-if="isShow==2" class="remcontent">
            <div class="remnav">
                <span>{{selected}}</span>
                <input class="amountInput" type="text" maxlength="11" placeholder="0.00" v-model.trim="amount">
            </div>
            <div class="remcont">
                <ul>
        ...
        <div class="remfooter">
            <div @click="zjyb" class="left">再记一笔</div>
...
export default {
    ...
    mounted(){
        ...
        if (this.$route.query.bookkeepingId) {
            this.bookkeepingId=this.$route.query.bookkeepingId;
            // this.billType
        }
        this.isShow1=1
            let now = new Date()
            this.currentDay = now.getDate() >= 10? now.getDate() : '0'+now.getDate()
        if (this.$route.query.bookType) {
            this.billType=this.$route.query.bookType;
            this.bookId=this.$route.query.bookId;
            this.currentYear=this.$route.query.currentYear;
            this.currentMonth=this.$route.query.currentMonth;
            this.currentMonth = (this.currentMonth>=10?this.currentMonth : this.currentMonth);
            this.createTimeStr = this.formatDate(this.currentYear,this.currentMonth,this.currentDay)
            // this.currentDay='01';
        }else{
            this.currentYear = now.getFullYear()
            this.currentMonth = (now.getMonth()+1)>=10?(now.getMonth()+1) : '0'+(now.getMonth()+1)
            this.createTimeStr = this.formatDate(this.currentYear,this.currentMonth,this.currentDay)
        }
        if (this.$route.query.name) {
            this.consumeType=this.$route.query.name
    ...
    //     next(vm =>{
    //         if(from.path == '/calendarInfo'){
    //             vm.bookkeepingId = vm.$route.query.bookkeepingId
    //         }
    //     })
    // },
    methods: {
